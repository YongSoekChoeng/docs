...
## <span style="color:green;">1. Docker 로컬 엔진 설정</span>

### 1.1 환경변수 설정
- `DOCKER_HOME` : `C:\DEV\CLOUD\Docker\20.10.10`
- `DOCKER_HOME` 을 `PATH` 변수에 추가
...
#### 1.2.1 설치 프로그램으로 설치
- 실행 파일: `Docker Desktop Installer.exe`
...
2. **Ubuntu 실행 (호환성 문제 발생 시 `winpty` 사용)**
   ```bash
   wsl -d Ubuntu -u root
   winpty wsl -d Ubuntu -u root
   ```

   - 호스트 PC의 `hypervisorlaunchtype` 설정 확인:
     ```bash
     ...
     - 결과가 `Auto` 가 아니라면 수정:
       ...
       ```
   - VirtualBox를 사용한다면 `off` 로 설정해야 함:
     ```bash
     bcdedit /set hypervisorlaunchtype off
     ```

3. **Ubuntu 업데이트 및 Docker 설치**
   ```bash
   apt update
...
## <span style="color:green;">2. Docker 클라이언트 기본 명령어</span>
...
#### 원격 접속
```bash
docker context create myserver --docker "host=ssh://root@<IP주소>"
...
## <span style="color:green;">3. 이미지 및 컨테이너 관리</span>
...
## <span style="color:green;">4. Dockerfile 예제</span>

### 4-1. Dockerfile 샘플(Spring Boot JAR)
```dockerfile
# 1. 베이스 이미지 선택
...
```
- **베이스 이미지 지정**
- 모든 Docker 이미지는 반드시 FROM으로 시작
- `eclipse-temurin:21-jdk`
  - OpenJDK 21
  - Spring Boot 3.x, Java 21 환경에 적합
- ❌ 운영 환경에서는 `-jre` 또는 `-alpine` 기반이 더 가벼움
...
```
- 컨테이너 내부의 **기본 작업 디렉토리**
- 이후 명령(COPY, RUN, CMD 등)은 모두 `/app` 기준
- ❌ WORKDIR 없이 상대경로 사용 → 가독성/유지보수 나쁨
...
```
	...
	|----|----|
	| COPY | 단순 복사 (권장) |
	| ADD | 압축 해제, URL 다운로드 가능 (불필요하면 사용 ❌) |
...
```
- 컨테이너가 **사용할 포트 문서화**
- 실제 포트 오픈은 `docker run -p` 또는 `docker-compose.yml`에서 수행
- ❌ EXPOSE만으로 포트가 열리지는 않음

##### 🔹 ENTRYPOINT
```dockerfile
ENTRYPOINT ["java", "-jar", "app.jar"]
```
- 컨테이너가 **실행될 때 반드시 수행되는 명령**
- JSON 배열 형식 → 권장 (쉘 문제 방지)
```
| 명령 | 용도 |
|----|----|
| ENTRYPOINT | 고정 실행 명령 |
...
### 4-3. ENTRYPOINT + CMD 조합 예제 (실무에서 가장 많이 사용)
```dockerfile
FROM eclipse-temurin:21-jdk
WORKDIR /app
COPY target/sample-app.jar app.jar
...
```
- 이미지 크기 대폭 감소
- 빌드 도구(Maven, Gradle) 포함 안 됨
- 보안 및 배포 효율 ↑
```

---

## <span style="color:green;">5. 이미지 빌드 및 컨테이너 실행</span>
...
## <span style="color:green;">6. 컨테이너 관리 명령어</span>
...
## <span style="color:green;">7. Docker Hub 연동</span>

```bash
docker login -u <계정명>
docker tag tomcat:10 <계정명>/tomcat:10
docker push <계정명>/tomcat:10
docker pull <계정명>/tomcat:10
```

---

## <span style="color:green;">8. 컨테이너 리소스 관리</span>
...
## <span style="color:green;">9. 컨테이너 Storage</span>
...
## <span style="color:green;">10. 네트워크 설정</span>
...
## <span style="color:green;">11. Docker Compose 예제</span>
...
services:
  mysql:
    ...
    #   - 이름만 적음 (경로 없음)
    #   - 실제 저장위치는 Docker가 관리(보통 호스트의 /var/lib/docker/volumes/<프로젝트명>_<볼륨명>/_data)
...
## <span style="color:green;">12. 컨테이너를 각각 개별로 띄웠을 경우 Network</span>

 두 compose 파일을 각각 docker-compose up 했다면 네트워크 이름이 같아도 프로젝트 이름이 다르면 네트워크가 분리될 수 있음. 이 경우 아래와 같은 솔루션이 있음.

### 12.1. 외부 네트워크 직접 생성 후 외부 네트워크 사용
...
## <span style="color:green;">13. dstone 프레임웍 적용</span>

### 13-1. 실행파일 생성
 - /dstone-boot/target 에 war 생성(dstone-boot 프로젝트 Maven 빌드)
 - /dstone-batch/target 에 jar 생성(dstone-batch 프로젝트 Maven 빌드)
 - /dstone-batch-server/target 에 jar 생성(dstone-batch-server 프로젝트의 bin/00.deploy.bat 실행)

### 13-2. 로컬에 workspace 작업구조 생성
 - /framework/docs/CLOUD/Docker/dstone적용/dstone-docker-workshop.bat 실행.
 - C:/Temp/workshop 아래에 호스트용 파일들 생성된 것 확인
 
### 13-3. 로컬 workspace 호스트로 복사
 - 생성된 C:/Temp/workshop/ 아래의 모든 자원을 호스트의 /workshop 아래로 복사.
 
### 13-4. dstone Docker workspace 작업구조
```text
/workshop
...
### 13-7. dstone 프레임웍 Docker Hub 등록
```bash
...
## <span style="color:green;">14. jenkins 적용</span>

### 14-1. 로컬에 workspace 작업구조 생성
 - /framework/docs/CLOUD/Docker/jenkins적용/03.dstone-jenkins-docker-workshop.bat 실행.
 - C:/Temp/workshop 아래에 호스트용 파일들 생성된 것 확인
 
### 14-2. 로컬 workspace 호스트로 복사
 - 생성된 C:/Temp/workshop/ 아래의 모든 자원을 호스트의 /workshop 아래로 복사.
 
### 14-3. jenkins Docker workspace 작업구조
```text
/workshop
...
##### 🔹 jenkins 기본 설정 (최초 1회)
   ...
   - Global Tool Configuration
      JDK: 컨테이너 내 JDK 21 이미 포함 → 설정 불필요
      Git: 기본 포함	  
   ```
...
📘 **정리 완료 (2025 버전)**  
이 문서는 Windows, WSL, 또는 Portable Docker 환경 모두에서 실행 가능한 표준화된 Docker 운영 매뉴얼입니다.
