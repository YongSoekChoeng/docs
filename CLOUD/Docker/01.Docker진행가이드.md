...
## 12. 컨테이너를 각각 개별로 띄웠을 경우 Network
...
### 13-1. 실행파일 생성
 - /dstone-boot/target 에 jar 생성(Maven 빌드)
 - /dstone-batch/target 에 jar 생성(Maven 빌드)
 - /dstone-batch-server/target 에 jar 생성(bin/00.deploy.bat 실행)

### 13-2. 로컬에 workspace 작업구조 생성
 - /framework/docs/CLOUD/Docker/02.프레임웍(dstone)적용/dstone-docker-workshop.bat 실행.
