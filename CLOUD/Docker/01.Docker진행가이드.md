...
| 상세 정보 | `docker inspect <컨테이너명>` |
| IP 확인 | `docker inspect --format '{{.NetworkSettings.IPAddress}}' <컨테이너명>` |
| 접속 | `docker exec -it <컨테이너명> /bin/bash` |
| 중지 | `docker stop <컨테이너명>` |
| 재시작 | `docker start <컨테이너명>` |
| 삭제 | `docker rm <컨테이너명>` |
| 이미지 목록 | `docker images` |
| 이미지 삭제 | `docker rmi <이미지명>` |
| 네트워크 목록 | `docker network ls` |
| 네트워크 삭제 | `docker network rm <네트워크이름>` |
