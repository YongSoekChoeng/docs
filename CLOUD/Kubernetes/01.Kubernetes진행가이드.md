...
[Cluster]
 ...
 │
 ├─ [Namespace : 논리적 단위]
 │    ├─ Deployment(Stateless 배포 논리단위)
 │    │    └─ ReplicaSet
 │    │         └─ Pod
 │    ├─ StatefulSet(Stateful 배포 논리단위)
 │    │    └─ Pod
 │    ├─ Service(네트워크 노출 논리단위)
 │    │    └─ Label Selector
 │    │         └─ Pod
...
- Cluster > Namespace > StatefulSet
```text
 1. 정의
   - Pod가 관리되는 상태저장(Stateful) 상위객체
 2. 특징
   ...
   - 파드들은 동일한 스팩으로 생성되었지만, 서로 교체는 불가능하다. 다시 말해, 각각은 재스케줄링 간에도 지속적으로 유지되는 식별자를 가진다.
   - 네트워크 식별자가 유지
   - Pod의 배포 / 업데이트 / 롤백 순차적으로 진행보장
   - 스토리지가 유지
 3. 주요 용도
   - 상태 저장 애플리케이션 (DB, 큐)
 4. Pod 이름
   - 고유하고 순차적 (-0, -1...)
 5. 네트워크
   - 고유한 DNS 이름 (Headless Service)
 6. 스토리지
   - 각 Pod별 독립 PVC (Volume Claim Template)
 7. 생성/삭제 순서
   - 순서 보장 (생성: 순차, 삭제: 역순)
```
...
- Cluster > Namespace > ConfigMap / PersistentVolumeClaim
```text
 1. 정의
   - 영구 저장소
 2. 특징
   - 영구 스토리지 볼륨을 설정하기 위한 객체
```

- Cluster > Namespace > ConfigMap / Secret
```text
 1. 정의
   - Pod에 대한 설정 관리 객체
...
### 7.1. Kubenetes Host 작업 구조

`dstone` 프레임워크는 아래와 같은 `workshop` 디렉토리 구조를 사용하여 Kubenetes Host 환경을 구성합니다.
...
/workshop
  ...
  └─ /dstone-batch-server                      # <Batch Admin Application>
     ├─ conf/                                  #   설정 파일
     └─ target/                                #   실행 파일
  └─ /dstone-kubenetes.yml                     # 전체 Kubenetes 빌드파일
```
